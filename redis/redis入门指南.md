...
我们必须明确，sort命令在默认的情况下是针对“值”来进行排序的！对于链表list和集合set我们很好理解，而对于有序集合它并不是根据有序集合中的元素分数进行排序，而是根据元素自身的值来进行排序，因而输出的效果也是不同于有序集合`zrange`命令本身的输出的！
